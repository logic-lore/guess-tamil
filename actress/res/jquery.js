levels = localStorage.tamilActressLevel
total = parseInt(tamilActressLevelTotal);
...
if (localStorage.tamActressTileList == null) {
	tile_list = []
	localStorage.tamActressTileList = JSON.stringify(tile_list);
}

tile_list = JSON.parse(localStorage.tamActressTileList);
...
$(document).ready(function () {
	...
	$('.skip-img').click(function () {
		...
		if (coins < skip_cost) {
			...
			coins -= skip_cost;
			levels++; localStorage.tamilActressLevel = levels;
	...
	$("#ans").click(function () {
		...
		setTimeout(() => {
			if (giveReward == 1) {
				//give one coin
				coins+= reward_coin; localStorage.setItem("tamilCoins", coins);
		...
		levels++;
		localStorage.tamilActressLevel = levels;
	});
